...
// Basic types (lightweight since project was JS)
interface UserRecord {
  ...
  createdAt?: string;
  password?: string; // dev fallback only
}
...
const UsersEdit: React.FC = () => {
  ...
  const [editCache, setEditCache] = useState<Record<string, Partial<UserRecord>>>({});
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      setUsers(list.sort((a: UserRecord,b: UserRecord)=> (a.username||'').localeCompare(b.username||'')));
    } catch (e:any) {
  ...
  const requestDelete = (id: string, username: string) => {
